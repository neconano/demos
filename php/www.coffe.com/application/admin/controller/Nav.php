...
EOF;
    ...
    public function add($group)
    {
        if (request()->isPost()) {
            ...
            // 获取前台模版供选择
            $template_list = get_template_list('nav');
            ...
            $builder->setMetaTitle('新增导航') // 设置页面标题
                ...
                ->addFormItem('pid', 'select', '上级导航', '上级导航', select_list_as_tree('AdminNav', array('group' => $group), '顶级导航'))
                ->addFormItem('title', 'text', '导航标题', '导航前台显示标题')
                ->addFormItem('type', 'radio', '导航类型', '导航类型', D2('AdminNav')->nav_type())
                ->addFormItem('url', 'text', '外链URL地址', '支持http://格式或者TP的U函数解析格式')
                ->addFormItem('module_name', 'select', '模块', '选择的模块需要具有前台页面', select_list_as_tree('AdminModule', null, null, 'name'), 'hidden')
    ...
    public function edit($id, $group)
    {
        if (request()->isPost()) {
            ...
            $builder->setMetaTitle('编辑导航') // 设置页面标题
                ...
                ->addFormItem('module_name', 'select', '模块', '选择的模块需要具有前台页面', select_list_as_tree('AdminModule', null, null, 'name'), $info['type'] === 'module' ? '' : 'hidden')
