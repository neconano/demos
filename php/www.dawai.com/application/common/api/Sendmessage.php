...
class Sendmessage extends Base
{
    ...
    {
        $code=Db::table('dw_tel_code')->where('phone','=',$phone)->order('id desc')->find();
        if ($code['time']+60>time()){
            return ['code'=>false,'msg'=>'请勿重复发送验证码'];
        }
        $code_num=Db::table('dw_tel_code')->where('phone','=',$phone)->whereTime('create_time','today')->count();
        if ($code_num>5){
        ...
        if (substr($arr,21,6) == 000000) {
            ...
            Db::table('dw_tel_code')->insert($data);
