 ...
 * 在创建完成后，通过Android Studio的Device File Explorer
 * 在data\data\package_name\files\datastore目录下看到一个名为name.preferences_pb的文件，其中保存了相关的键值对。
 */
...
object DataStoreUtils {
    ...
    fun readBooleanFlow(key: String, default: Boolean = false): Flow<Boolean> =
        dataStore.data
            .catch {
                //当读取数据遇到错误时，如果是 `IOException` 异常，
                // 发送一个 emptyPreferences 来重新使用
                ...
                // map 直接emit高阶函数返回值
                //字段的key，boolean 型需要使用 booleanPreferencesKey 包装一下
                it[booleanPreferencesKey(key)] ?: default
            }

    /**
     * 这要从 suspend 修饰符说起，协程使用中可以使用该修饰符修饰一个函数，
     * 表示该函数为挂起函数，从而运行在协程中。 挂起函数，它不会造成线程阻塞，
     * 但是会 挂起 协程，并且只能在协程中使用。挂起函数不可以在main函数中被调用，
     * 那么我们怎么调试呢？对了，就是使用runBlocking 函数！
     * first是挂起函数，map是内联函数
     * first执行逻辑是如果高阶函数返回的是true,则first返回高阶函数结果
     */
