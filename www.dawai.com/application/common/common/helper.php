...
if (!function_exists('model')) {
    ...
    function model($name = '', $layer = 'model', $appendSuffix = false)
    {
        ...
        if (strpos($name, '://')) {
            ...
            if ($extend == 'Addons') {
                // 扩展资源
                $class = '\\addon\\' . lcfirst($model[0]) . '\\model' . '\\' . ucfirst($model[1]);
                return new $class;
            } else {
                // 扩展资源
                $class = '\\' . $extend . '\\' . lcfirst($model[0]) . '\\model' . '\\' . ucfirst($model[1]);
                return new $class;
            }
        } else {
    ...
    function action($url, $vars = [], $layer = 'controller', $appendSuffix = false)
    {
        ...
        if (strpos($url, '://')) {
            ...
            if (count($model) == 2) {
                if ($extend == 'Addons') {
                    // 扩展资源
                    $class = '\\addon\\' . lcfirst($model[0]) . '\\' . $layer . '\\' . ucfirst($model[1]);
                    return new $class;
                } else {
                    // 扩展资源
                    $class = '\\' . $extend . '\\' . lcfirst($model[0]) . '\\' . $layer . '\\' . ucfirst($model[1]);
                    return new $class;
                }
            }
        } else {
