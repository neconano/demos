...
class WeixinBank
{
    ...
    public function __construct($params){
        ...
        $this->mch_appid = $wechat['app_id'];
        $this->mch_id = $payment['merchant_id'];
        $this->key = $payment['key'];
        $this->certificate_path = getcwd().'/cert';
    }
    /**
     * [提现到微信零钱]
     ...
     */
    public function paybank($bank_no, $true_name, $bank_code, $amount, $partner_trade_no, $desc='企业转账')
    {
        $paybank = [
            'mch_id'            => $this->mch_id,
            'partner_trade_no'  => $partner_trade_no,
    ...
    protected static function getRSA($string, $certificate_path){
        $publicKey = file_get_contents($certificate_path.'/public.pem');
        $encryptedBlock = '';
